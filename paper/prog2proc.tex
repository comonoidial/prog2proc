...
As a demonstration we use the hardware implemention on an FPGA of the signal processing for a laser range finder.
...
\textit{TODO be more concrete about what clash is} \\

Designing hardware architectures for FPGA is traditionally done using low-level hardware description languagles (HDLs) like Verilog or VHDL.
\clash \cite{Baaij} is a subset of Haskell that can be compiled to widely supported lower level HDLs. Even though you can use most abstractions that Haskell has to offer, \clash is still a structural hardware description language. \\

\textit{TODO mention fixed sized vectors, signals and other clash features used in this paper}
...
most step have a lot of data copied from one continuation to another \\
and stack is very wide because some continuations capture a lot of free variables \\
only a few variables are used every cycle \\
...
as seen from the transformation process function calls and return form natural boundary between steps \\
...
for some smaller function the added cycle boundaries cause too much overhead \\
we use the inline wrapper (that does nothing) to make explicit that a function is used without any overhead \\
...
iterative closest point \cite{ICP} is an algorithm used for position finding using a laser range finder \\

To demonstrate the idea's presented in this paper we take the Iterative Closest Point (ICP) as an example case. ICP is an algorithm that minimizes the distance between two point sets. In robotics the algorithm is often used to determine the robot movement based on sensor data.
...
Fig.~\ref{fig:icp_scans} shows an example of this where robot takes a scan of the environment (\ref{fig:icp_scanA}), moves an unknown distance, and then takes a second scan of the environment\ref{fig:icp_scanB}, by aligning the two scans the moved distance is calculated.
An example of this is shown in Figure~\ref{fig:icp_scans}, a more detailed description is given in \cite{Robin:Hendrik}.
\begin{figure}[ht]
	\centering
...
some remarks about the differences between the implementations and effort from hardware designer \\
compare hardware implementation with the generated results from this section
...
control unit keep track of loop counters to support zero overhead loop constructs
