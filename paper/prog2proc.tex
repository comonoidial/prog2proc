...
The past decades we got hardware performance gains first from higher clock frequencies and later from increased parallelism.
...
This leaves specialisation of hardware designs as the key method to gain efficiency and performance.

The amount of logic gates available on a chip is still increasing, allowing complex algorithms to be directly implemented in hardware.
...
%\textit{To be explained more, this key step in the process}
...
\draw[->, densely dotted] ([xshift=-3em] mc.south) -- ([xshift=1em] cc.north);
